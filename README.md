...
omaterm new hand --docker-access --prepare --exec 'mkdir -p Work/basecamp && cd Work/basecamp && gh repo clone basecamp/bc3 && cd bc3 && setup --reset'
...
Use `--prepare` with `--exec` to start a named Omaterm in the background and wait until setup plus the bootstrap command finish. The container stays running afterward so it can receive `omaterm exec NAME ...` commands.
